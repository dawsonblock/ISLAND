...
ARfsnSampleMerchant::ARfsnSampleMerchant()
{
	...
	DialogueTrigger = CreateDefaultSubobject<URfsnNpcDialogueTrigger>(TEXT("DialogueTrigger"));
...
void ARfsnSampleMerchant::BeginPlay()
{
	...
	if (RfsnClient)
	{
		...
		// Bind events
		RfsnClient->OnNpcActionReceived.AddDynamic(this, &ARfsnSampleMerchant::OnRfsnAction);
		RfsnClient->OnSentenceReceived.AddDynamic(this, &ARfsnSampleMerchant::OnDialogueSentence);
	}
	...
	if (DialogueTrigger)
	{
		...
		DialogueTrigger->TriggerCooldown = 8.0f;
	}

	UE_LOG(LogTemp, Log, TEXT("[Merchant] %s ready for trading with %d items"), *RfsnClient->NpcName,
	       InventoryItems.Num());
}
