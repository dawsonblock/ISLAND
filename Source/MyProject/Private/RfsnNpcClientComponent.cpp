...
void URfsnNpcClientComponent::BeginPlay() { Super::BeginPlay(); }

void URfsnNpcClientComponent::EndPlay(
...
void URfsnNpcClientComponent::SendPlayerUtterance(const FString &PlayerText) {
  ...
  CurrentRequest->OnRequestProgress().BindUObject(
      this, &URfsnNpcClientComponent::OnStreamProgress);
  CurrentRequest->OnProcessRequestComplete().BindUObject(
      this, &URfsnNpcClientComponent::OnStreamComplete);
  ...
  UE_LOG(LogTemp, Log, TEXT("[RFSN] Sending utterance to %s: %s"), *NpcName,
         *PlayerText);
...
void URfsnNpcClientComponent::OnStreamProgress(FHttpRequestPtr Request,
                                               int32 BytesSent,
                                               int32 BytesReceived) {
  if (!Request.IsValid()) {
    return;
...
void URfsnNpcClientComponent::OnStreamComplete(FHttpRequestPtr Request,
  ...
  if (!bSuccess || !Response.IsValid()) {
    ...
    if (Response.IsValid()) {
      ErrorMsg =
          FString::Printf(TEXT("HTTP %d: %s"), Response->GetResponseCode(),
  ...
  UE_LOG(LogTemp, Log, TEXT("[RFSN] Dialogue stream complete for %s"),
         *NpcName);
...
void URfsnNpcClientComponent::ParseMetaEvent(const FString &JsonData) {
  ...
  if (!FJsonSerializer::Deserialize(Reader, JsonObject) ||
      !JsonObject.IsValid()) {
    UE_LOG(LogTemp, Warning, TEXT("[RFSN] Failed to parse meta event: %s"),
           *JsonData);
    return;
  }
  ...
  UE_LOG(LogTemp, Log, TEXT("[RFSN] Meta: action=%s, mode=%s, signal=%s"),
         *ActionString, *Meta.ActionMode, *Meta.PlayerSignal);
...
void URfsnNpcClientComponent::ParseSentenceEvent(const FString &JsonData) {
  ...
  if (!FJsonSerializer::Deserialize(Reader, JsonObject) ||
      !JsonObject.IsValid()) {
    UE_LOG(LogTemp, Warning, TEXT("[RFSN] Failed to parse sentence event: %s"),
           *JsonData);
    return;
  }
...
ERfsnNpcAction
URfsnNpcClientComponent::ParseNpcAction(const FString &ActionString) {
