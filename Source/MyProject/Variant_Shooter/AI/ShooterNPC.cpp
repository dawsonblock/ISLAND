...
void AShooterNPC::BeginPlay() {
  ...
  // Create RFSN client component
  RfsnClient = NewObject<URfsnNpcClientComponent>(this, TEXT("RfsnClient"));
  if (RfsnClient) {
    RfsnClient->RegisterComponent();
    RfsnClient->NpcName = GetName();
    RfsnClient->NpcId = FString::Printf(TEXT("shooter_npc_%s"), *GetName());
    RfsnClient->OnNpcActionReceived.AddDynamic(this,
                                               &AShooterNPC::OnRfsnNpcAction);
  }
...
float AShooterNPC::TakeDamage(float Damage,
...
void AShooterNPC::AttachWeaponMeshes(AShooterWeapon *WeaponToAttach) {
  const FAttachmentTransformRules AttachmentRule(EAttachmentRule::SnapToTarget,
                                                 false);
  ...
  WeaponToAttach->GetFirstPersonMesh()->AttachToComponent(
      GetFirstPersonMesh(), AttachmentRule, FirstPersonWeaponSocket);
  WeaponToAttach->GetThirdPersonMesh()->AttachToComponent(
      GetMesh(), AttachmentRule, ThirdPersonWeaponSocket);
}
...
FVector AShooterNPC::GetWeaponTargetLocation() {
  ...
  if (CurrentAimTarget) {
    ...
    AimDir = UKismetMathLibrary::RandomUnitVectorInConeInDegrees(
        AimDir, AimVarianceHalfAngle);

  } else {

    // no aim target, so just use the camera facing
    AimDir = UKismetMathLibrary::RandomUnitVectorInConeInDegrees(
        GetFirstPersonCameraComponent()->GetForwardVector(),
  ...
  GetWorld()->LineTraceSingleByChannel(OutHit, AimSource, AimTarget,
...
void AShooterNPC::DeferredDestruction() { Destroy(); }
...
void AShooterNPC::SpeakToPlayer(const FString &PlayerUtterance) {
  if (RfsnClient) {
    RfsnClient->SendPlayerUtterance(PlayerUtterance);
  }
}

void AShooterNPC::OnRfsnNpcAction(ERfsnNpcAction Action) {
  // React to RFSN action selection
  switch (Action) {
  case ERfsnNpcAction::Attack:
  case ERfsnNpcAction::Threaten:
    // Go hostile - could trigger StateTree behavior
    UE_LOG(LogTemp, Log,
           TEXT("[%s] RFSN action: Attack/Threaten - becoming hostile"),
           *GetName());
    break;
  ...
  case ERfsnNpcAction::Flee:
    ...
    StopShooting();
    UE_LOG(LogTemp, Log, TEXT("[%s] RFSN action: Flee - disengaging"),
           *GetName());
    break;
  ...
  case ERfsnNpcAction::Trade:
    // Friendly behavior
    StopShooting();
    UE_LOG(LogTemp, Log, TEXT("[%s] RFSN action: Friendly - lowering weapon"),
           *GetName());
    break;

  default:
    // Default behavior
    UE_LOG(LogTemp, Log, TEXT("[%s] RFSN action: %d"), *GetName(),
