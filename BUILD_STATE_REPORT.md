...
| Runtime-root targeted backend slice | `cd RFSN_NPC_AI/Python && RFSN_RUNTIME_ROOT="$(mktemp -d)" python3 -m pytest tests/test_runtime_paths.py tests/test_integration.py tests/test_memory_manager.py tests/test_memory_leaks.py -q` | passed | `33 passed, 1 skipped` under an isolated runtime root |
| Python backend | `cd RFSN_NPC_AI/Python && RFSN_RUNTIME_ROOT="$(mktemp -d)" python3 -m pytest --continue-on-collection-errors -q` | passed | `261 passed, 1 skipped` under an isolated runtime root |
...
- Full Unreal-integrated dialogue validation still depends on local in-editor/runtime verification with the backend attached.
...
The repaired repo is materially healthier than the earlier failure snapshot. The remaining gaps are no longer repo-layout, TTL, or runtime-path regressions; they are the environment-dependent Unreal checks and full end-to-end dialogue validation inside a local Unreal session.
